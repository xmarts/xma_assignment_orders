...
// models.load_fields('product.product', ['sub_type_uom']);
    ...
    models: [
    {
      ...
      loaded: function(self, picking_type) {
        ...
        fields: ['display_name', 'lst_price', 'standard_price', 'categ_id', 'pos_categ_id', 'taxes_id',
                 'barcode', 'default_code', 'to_weight', 'uom_id', 'sub_type_uom', 'quantity_uom', 'description_sale', 'description',
...
models.register_payment_method = function(use_payment_terminal, ImplementedPaymentInterface) {
    models.PosModel.prototype.electronic_payment_interfaces[use_payment_terminal] = ImplementedPaymentInterface;
};
...
//     var models = models.PosModel.prototype.models;
...
models.Orderline = models.Orderline.extend({
    ...
    clone: function(){
        ...
        orderline.quantity_caja = this.quantity_caja;
        orderline.quantityStr = this.quantityStr;
        orderline.quantityStrCaja = this.quantityStrCaja;
    ...
    set_quantity: function(quantity, keep_price){
        ...
        if(quantity === 'remove'){
            ...
            if(unit){
                if (unit.rounding) {
                    this.quantity = 0.00;
                    this.quantityStr = 0.00;
                    this.quantity_caja = 0.00;
                    this.quantityStrCaja = 0.00;
                    var decimals = this.pos.dp['Product Unit of Measure'];
                    var rounding = Math.max(unit.rounding, Math.pow(10, -decimals));
                    var unidades_libres = 0;
                    var get_count_caja = parseInt(this.get_count_caja());
                    if(quant >= get_count_caja){
                        var cajas = parseInt(quant/get_count_caja);
                        this.quantity_caja = round_pr(cajas, rounding);
                        this.quantityStrCaja = field_utils.format.float(this.quantity_caja, {digits: [69, decimals]});
                        var unidades = cajas * get_count_caja
                        if(unidades < quant){
                            unidades_libres = quant - unidades
                            this.quantity = round_pr(unidades_libres, rounding);
                    ...
                    this.quantity_caja    = round_pr(quant, 1);
                    this.quantityStr = this.quantity.toFixed(0);
                    this.quantityStrCaja = this.quantity_caja.toFixed(0);
                }
            }
            else{
                this.quantity    = quant;
                this.quantity_caja    = quant;
                this.quantityStr = '' + this.quantity;
                this.quantityStrCaja = '' + this.quantity_caja;
            }
        }
    ...
    get_quantity: function(){
        return this.quantity + (this.get_quantity_caja() * this.get_count_caja());
    },
    // return the quantity caja of product
    get_quantity_caja: function(){
        return this.quantity_caja;
    },
    get_quantity_str: function(){
        return this.quantityStr;
    },
    get_quantity_str_caja: function(){
        return this.quantityStrCaja;
    },
    ...
    get_count_caja: function(){
        return this.product.get_count_caja();
    },
    ...
    export_for_printing: function(){
        return {
            ...
            // quantity_caja:      this.get_quantity_caja(),
            unit_name:          this.get_unit().name,
            // unit_name_caja:     this.get_unit_caja().name,
...
models.Packlotline = models.Packlotline.extend({
    ...
    add: function(){
        ...
        var new_lot_model = new models.Packlotline({}, {'order_line': this.order_line});
...
models.Paymentline = models.Paymentline.extend({
    ...
    */
    ...
    //models as JSON for server communication
    export_as_JSON: function(){
        return {
    ...
    //models as JSON for receipt printing
    export_for_printing: function(){
        return {
            cid: this.cid,
            amount: this.get_amount(),
